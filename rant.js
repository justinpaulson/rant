...
if (Meteor.isServer) {
  ...
  Meteor.methods({getAllPosts: function () {
    ...
    Posts.find({}).fetch().forEach(function (element, index, array) {
      ...
      posts.push(_.extend(element, {url:'/blog/posts/'+element._id._str, category_color: category.color, category_image: category.image}));
    });
    return posts;
  }});
  ...
  Meteor.methods({getPost: function (id) {
    ...
    return _.extend(post, {url:'/blog/posts/'+id, category_color: category.color, category_image: category.image});
  }});
}
